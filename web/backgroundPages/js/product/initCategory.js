...
(function($){
	var _this;
	var _options;
	$.fn.initCategorys=function(options){
		_options=options;
		var defaults={
				offset:0,
				limit:20
	...
	function createTable(ob,url){
		...
		if(_temp.length){
			for(var i=0;i<_temp.length;i++){
				...
				$tbody=$tbody+"<td align='center'><input type='checkbox' name='category_id' value='"+_temp[i].category_id+"'></td>";
				$tbody=$tbody+"<td align='center'>"+_temp[i].category_name+"</td>";
				$tbody=$tbody+"<td align='center'>"+_temp[i].category_remark+"</td>";
				$tbody=$tbody+"</tr>";
			}
		}
