 $(document).ready(function(){
            	...
            	var ob=eval("("+result+")");
            	 if(ob.length){
            		 ...
            		 for(var i=0;i<ob.length;i++){
      					...
      					$tbody=$tbody+"<td align='center'><input type='checkbox' name='news_id' value='"+ob[i].category_id+"'></td>";
      					$tbody=$tbody+"<td align='center'>"+ob[i].category_name+"</td>";
      					$tbody=$tbody+"<td align='center'>"+ob[i].category_sort+"</td>";
      					$tbody=$tbody+"<td align='center'>"+ob[i].category_detail+"</td>";
    	...
    	$("#addBtn").click(function(){
    		 //先获取最大sort
    		 $.ajax({
    			 url:MyUtil.getContext()+"getMaxSort.do",
    			 data:{},
    			 type:"get",
    	      	 async:true,
    	         success:function(result){
    	...
    	$("#delBtn").del({
    		...
    		ACTION:MyUtil.getContext()+"deleteNewsCatergory.do"
    	});
    	//全选
    ...
    function save(obj){
    	...
    	var ctname=_otr.find("input[name='ctname']").val();
   	    ...
   	    $.ajax({
   	    	url:MyUtil.getContext()+"saveOrUpdateNewsCatergory.do",
   	    	data:{
   	    	 "category_id":cid,
      		 "category_name":ctname,
      		 "category_sort":ctsort,
      		 "category_detail":ctdetail
      		},
      		type:"post",
      		async:true,
            success:function(result){
				...
				_tr=_tr+"<tr class='even' style='background-color:#fff;font-size:14px;font-style:normal;font-family:Arial;' >";	
   	    		...
   	    		_tr=_tr+"<td align='center'><input type='checkbox' name='news_id' value='"+ob.category_id+"'></td>";
   	    		_tr=_tr+"<td align='center' width='100'>"+ob.category_name+"</td>";
   	    		_tr=_tr+"<td align='center' width='100'>"+ob.category_sort+"</td>";
   	    		_tr=_tr+"<td align='center'>"+ob.category_detail+"</td>";
